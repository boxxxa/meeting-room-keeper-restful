...
API_NAME = 'Meeting Room Keeper project. RESTful API'
API_VERSION = '0.1.0'
...
SQL_DEFAULT_FIELDS='''items.id, items.name, items.description, strftime('%Y-%m-%dT%H:%M:%SZ', start_date) as start_date,
strftime('%Y-%m-%dT%H:%M:%SZ', end_date) as end_date, place_id, user_id, itemtype_id'''
if DEBUG:
    SQL_DEBUG = True
else:
    SQL_DEBUG = False